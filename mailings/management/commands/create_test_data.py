...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f"✓ Создано сообщений: {len(messages)}"))

        # Создаем рассылки
        mailings = self.create_test_mailings(mailings_count, users, clients, messages)
        self.stdout.write(self.style.SUCCESS(f"✓ Создано рассылок: {len(mailings)}"))
    ...
    def create_test_messages(self, count, users):
        ...
        for i in range(count):
            ...
            message = Message.objects.create(subject=subject, body=body, owner=owner)
        ...
        for i in range(1, count + 1):
            ...
            if owner_clients:
                ...
                selected_clients = random.sample(
                    owner_clients, random.randint(1, min(len(owner_clients), 10))
                )
