...
from mailings.cache import invalidate_mailing_cache, invalidate_user_cache
    ...
    def handle(self, *args, **options):
        ...
        if mailing_id:
            try:
                ...
                self.stdout.write(self.style.ERROR("ID рассылки должен быть числом"))
                return
        else:
        ...
        for mailing in mailings:
            ...
            if not dry_run:
                if mailing.status == Mailing.STATUS_CREATED:
                    ...
                    mailing.save()
                    if verbose:
                        self.stdout.write(f'  ✓ Статус рассылки изменен на "Запущена"')
                ...
                if current_time >= mailing.end_datetime:
                    ...
                    mailing.save()
                    if verbose:
                        self.stdout.write(f'  ✓ Статус рассылки изменен на "Завершена"')
