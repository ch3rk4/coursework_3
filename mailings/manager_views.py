...
from .cache import invalidate_mailing_cache, invalidate_user_cache
...
class ManagerMailingListView(ManagerRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = Mailing.objects.select_related("owner", "message").order_by(
            "-created_at"
        )
...
def toggle_user_active(request, user_id):
    ...
    if user == request.user or user.is_superuser:
        return JsonResponse(
            {"error": "Нельзя заблокировать этого пользователя"}, status=400
        )
...
def toggle_mailing_status(request, mailing_id):
    ...
    if mailing.owner == request.user:
        return JsonResponse(
            {"error": "Используйте обычное редактирование для своих рассылок"},
            status=400,
        )
